...
const Edit = (props) => {
  ...
  return (
    <>
      <EditUI handleSubmit={(values) => update({ id: values.id, description: values.description, end_date: moment(values.end_date).format('YYYY-MM-DD'), start_date: moment(values.start_date).format('YYYY-MM-DD'), frequency: values.frequency, name: values.name, employees_id: values.employees_id }, `${url}/${values.id}`)} {...props} participants={dataParticipants} employees={data} />
