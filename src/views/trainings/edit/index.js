...
const Edit = (props) => {
  ...
  return (
    <>
      <EditUI handleSubmit={(values) => update({ description: values.description, end_data: moment(values.end_date).format('YYYY-MM-DD'), start_date: moment(values.start_date).format('YYYY-MM-DD'), frequency: values.frequency, name: values.name }, `${url}/${values.id}`)} {...props} employees={data} />
